// ===================================
// COB CORN POS - UPDATED SYSTEM V2
// With all new changes from feedback
// ===================================
...
// Enhanced Inventory - NOW includes Cups
let inventory = JSON.parse(localStorage.getItem('cobCornInventory')) || {
    'sweet-corn': { 
        name: 'Sweet Corn', unit: 'grams', openingStock: 10000, totalStock: 10000, 
        purchases: 0, consumedToday: 0, abnormalLoss: 0,
        lastPurchaseDate: '', lastLossDate: '', cost: 0, minAlert: 1000
    },
    'nachos': { 
        name: 'Nachos', unit: 'grams', openingStock: 1400, totalStock: 1400,
        purchases: 0, consumedToday: 0, abnormalLoss: 0,
        lastPurchaseDate: '', lastLossDate: '', cost: 0, minAlert: 500
    },
    'cups': { 
        name: 'Cups', unit: 'pieces', openingStock: 500, totalStock: 500,
        purchases: 0, consumedToday: 0, abnormalLoss: 0,
        lastPurchaseDate: '', lastLossDate: '', cost: 0, minAlert: 50
    }
};

// Expenses
let expenses = JSON.parse(localStorage.getItem('cobCornExpenses')) || [];
let expenseCategories = JSON.parse(localStorage.getItem('cobCornExpenseCategories')) || [
    'Rent', 'Utilities', 'Supplies', 'Wages', 'Marketing', 'Other'
];
...
let syncStatus = { isSyncing: false, lastSync: null, isConnected: false };
...
function checkDailyReset() {
    ...
    if (lastDate && lastDate !== today) {
        ...
        localStorage.setItem('cobCornTodayCash', '0');
...
function applyTabLocking() {
    ...
    document.querySelectorAll('.tab-btn').forEach(btn => {
        ...
        if (lockedTabs.includes(tabName)) {
            ...
            btn.title = '🔒 Locked - Admin access required';
        }
    });
}
...
function testGoogleConnection() {
    ...
    .catch(() => showMessage('Test sent', 'warning'));
}

function syncAllDataToGoogle() {
    if (!CONFIG.googleAppsScriptURL || syncStatus.isSyncing) return;

    syncStatus.isSyncing = true;
    const today = getTodayDateString();

    // CHANGED: Only sync fulfilled orders to Google Sheets
    const fulfilledOrders = orders.filter(order => order.status === 'fulfilled');

    const dataToSync = {
        action: 'syncAllData',
        orders: fulfilledOrders, // Only fulfilled orders
        inventory: inventory,
        expenses: expenses,
    ...
    .catch(() => syncStatus.isSyncing = false);
}
...
function loadQuickItems() {
    ...
    menuItems.forEach(item => {
        ...
        btn.innerHTML = `<div class="quick-item-name">${item.name}</div><div class="quick-item-price">₹${item.price}</div>`;
...
function loadMenuListSettings() {
    ...
    menuItems.forEach(item => {
        ...
        div.innerHTML = `
            ...
            <div style="font-size:12px; color:#666; margin-bottom:8px;">Price: ₹${item.price} | Consumption: ${item.consumptionPerUnit}g/${item.inventoryId === 'sweet-corn' ? 'cup' : (item.inventoryId === 'nachos' ? 'plate' : 'unit')}</div>
            <div style="display:grid; grid-template-columns:1fr 1fr; gap:8px;">
                <button onclick="editMenuItem('${item.id}')" style="padding:6px 12px; background:#2196F3; color:white; border:none; border-radius:4px; cursor:pointer; font-size:11px; font-weight:600;">✏️ Edit</button>
                <button onclick="deleteMenuItem('${item.id}')" style="padding:6px 12px; background:#d32f2f; color:white; border:none; border-radius:4px; cursor:pointer; font-size:11px; font-weight:600;">🗑️ Delete</button>
            </div>
        `;
...
function editMenuItem(itemId) {
    ...
    const newConsumption = prompt('Consumption Per Unit (grams):', item.consumptionPerUnit);
    if (newConsumption === null) return;

    item.name = newName.trim();
    item.price = parseFloat(newPrice);
    item.consumptionPerUnit = parseFloat(newConsumption);
...
function loadInventoryItemsList() {
    ...
    // Update dropdown in Add Menu Item
...
function addInventoryItem() {
    ...
    inventory[key] = {
        ...
        lastLossDate: '',
        cost: 0,
        minAlert: Math.max(10, qty * 0.1) // 10% of opening as alert
    };
...
function deleteInventoryItem(key) {
    // Check if any menu item uses this inventory
...
function loadInventoryDisplay() {
    ...
    Object.entries(inventory).forEach(([key, item]) => {
        ...
        const status = available <= item.minAlert ? '⚠️ LOW' : '✓ OK';
        ...
        card.innerHTML = `
            ...
            <div style="background:#f8f9fa; padding:15px; border-radius:5px; margin-bottom:15px; font-size:13px;">
                ...
                ${item.lastPurchaseDate ? `<div style="font-size:11px; color:#999; padding:0 0 8px 20px;">Last: ${item.lastPurchaseDate}</div>` : ''}
                <div style="display:flex; justify-content:space-between; padding:8px 0; border-bottom:1px dashed #ddd;">
                ...
                ${item.lastLossDate ? `<div style="font-size:11px; color:#999; padding:0 0 8px 20px;">Last: ${item.lastLossDate}</div>` : ''}
            ...
            <div style="display:grid; grid-template-columns:1fr 1fr 1fr; gap:8px;">
                <button onclick="addStock('${key}')" class="btn btn-success" style="padding:8px; font-size:12px;">
                    ➕ Add
                </button>
                <button onclick="recordLoss('${key}')" style="padding:8px; background:#ff9800; color:white; border:none; border-radius:5px; cursor:pointer; font-size:12px;">
                    📝 Loss
                </button>
                <button onclick="editTotalStock('${key}')" style="padding:8px; background:#2196F3; color:white; border:none; border-radius:5px; cursor:pointer; font-size:12px;">
                    ✏️ Edit
                </button>
            </div>
        `;
...
function addStock(invKey) {
    ...
    loadInventoryDisplay();
    showMessage(`✓ Added ${qty} ${inventory[invKey].unit} to ${inventory[invKey].name}`, 'success');

    if (syncStatus.isConnected) syncAllDataToGoogle();
}
...
function recordLoss(invKey) {
    ...
    loadInventoryDisplay();
    showMessage(`✓ Recorded loss: ${loss} ${inventory[invKey].unit} from ${inventory[invKey].name}`, 'warning');
...
function editTotalStock(invKey) {
    ...
    localStorage.setItem('cobCornInventory', JSON.stringify(inventory));
    loadInventoryDisplay();
    showMessage(`✓ Stock updated to ${qty} ${inventory[invKey].unit}`, 'success');

    if (syncStatus.isConnected) syncAllDataToGoogle();
}

// CHANGED: Deduct inventory only when order is fulfilled
function deductInventory(order) {
    order.items.forEach(orderItem => {
        ...
        // NEW: Deduct cups for sweet corn items
        if (menuItem.inventoryId === 'sweet-corn' && inventory['cups']) {
            inventory['cups'].consumedToday += orderItem.quantity; // 1 cup per sweet corn item
        }
    });
...
function updateOrderDisplay() {
    ...
    currentOrder.items.forEach((item, idx) => {
        html += `<tr style="border-bottom:1px solid #eee;"><td style="padding:8px;">${item.name}</td><td style="text-align:center; padding:8px;"><input type="number" value="${item.quantity}" min="1" style="width:50px; text-align:center; padding:4px; border:1px solid #ddd; border-radius:3px;" onchange="updateItemQuantity(${idx}, this.value)"></td><td style="text-align:right; padding:8px;">₹${item.total}</td><td style="padding:8px;"><button onclick="removeItem(${idx})" style="color:white; background:#ff4444; border:none; padding:4px 10px; cursor:pointer; font-size:11px; border-radius:3px;">✕</button></td></tr>`;
    });
    html += '</table>';
...
// CHANGED: Payment Received now triggers save
function togglePaymentReceivedAndSave() {
    ...
    if (currentOrder.paymentReceived) {
        ...
        // Auto-save when payment received
        saveOrder();
    } else {
        btn.classList.remove('active');
        btn.innerHTML = '⏱ Payment Pending';
    }
}

// CHANGED: Simplified save - always saves as pending
function saveOrder() {
    if (currentOrder.items.length === 0) {
    ...
    const order = {
        ...
        status: 'pending', // Always pending on save
        customerName: customerName,
        specialNotes: specialNotes,
        date: getTodayDateString()
    };

    // NOTE: Inventory NOT deducted here - only when fulfilled

    orders.push(order);
    localStorage.setItem('cobCornOrders', JSON.stringify(orders));

    if (paymentMethod === 'cash' && currentOrder.paymentReceived) {
        let todayCash = parseFloat(localStorage.getItem('cobCornTodayCash')) || 0;
        todayCash += order.total;
        localStorage.setItem('cobCornTodayCash', todayCash);
        document.getElementById('todayCash').textContent = todayCash.toFixed(2);
    }

    showMessage('✓ Order saved: ' + orderNumber, 'success');
...
function resetForm() {
    ...
    if (paymentBtn) {
        ...
        paymentBtn.innerHTML = '⏱ Payment Pending';
    }
...
// ============================================
// PENDING ORDERS (IN NEW ORDER TAB)
// ============================================
...
function loadPendingOrdersInline() {
    ...
    if (pendingOrders.length === 0) {
        container.innerHTML = '<div style="text-align:center; color:#999; padding:20px; font-size:12px;">📭 No pending orders</div>';
        return;
    }
    ...
    pendingOrders.forEach((order) => {
        ...
        orderCard.innerHTML = `
            <div style="display:flex; justify-content:space-between; margin-bottom:6px;">
                ...
                <span style="background:#fff; padding:3px 10px; border-radius:12px; font-size:10px; border:1px solid #e63946; color:#e63946;">⏱ PENDING</span>
            </div>
            ...
            <div style="color:#666; margin-bottom:8px; font-size:11px;">Amount: ₹${order.total.toFixed(2)} | ${order.paymentMethod.toUpperCase()}</div>
            <div style="display:grid; grid-template-columns:1fr 1fr; gap:8px;">
                <button onclick="markFulfilledInline('${order.orderNumber}')" style="padding:6px; background:#06a77d; color:white; border:none; border-radius:4px; cursor:pointer; font-size:11px; font-weight:600;">
                    ✓ Fulfill
                </button>
                <button onclick="cancelOrderInline('${order.orderNumber}')" style="padding:6px; background:#d32f2f; color:white; border:none; border-radius:4px; cursor:pointer; font-size:11px; font-weight:600;">
                    ❌ Cancel
                </button>
            </div>
        `;
...
// CHANGED: Mark fulfilled now deducts inventory and syncs to Google Sheets
function markFulfilledInline(orderNumber) {
    const order = orders.find(o => o.orderNumber === orderNumber);
    if (order) {
        order.status = 'fulfilled';

        // DEDUCT INVENTORY NOW (not on save)
        deductInventory(order);

        localStorage.setItem('cobCornOrders', JSON.stringify(orders));
        loadPendingOrdersInline();
        showMessage('✓ Order fulfilled - Inventory updated', 'success');

        // Sync to Google Sheets (fulfilled orders only)
        if (syncStatus.isConnected) syncAllDataToGoogle();
    }
}

// CHANGED: Cancel order now syncs to Google Sheets
...
function loadEndOfDayData() {
    const today = getTodayDateString();
    const todayOrders = orders.filter(order => order.date === today && order.status === 'fulfilled'); // Only fulfilled
...
function finalizeEOD() {
    ...
    const eodData = {
        date: today,
        systemTotal: parseFloat(document.getElementById('eodSystemTotal').textContent),
        cashSales: parseFloat(document.getElementById('eodCashSales').textContent),
        onlineSales: parseFloat(document.getElementById('eodOnlineSales').textContent),
        actualCash: parseFloat(document.getElementById('eodActualCash').value) || 0,
        cashVariance: parseFloat(document.getElementById('eodCashVariance').textContent) || 0,
        inventory: {},
        cupsSystem: parseInt(document.getElementById('eodCupsSystem').textContent) || 0,
        cupsPhysical: parseInt(document.getElementById('eodCupsPhysical').value) || 0,
        cupsVariance: parseInt(document.getElementById('eodCupsVariance').textContent) || 0,
        platesSystem: parseInt(document.getElementById('eodPlatesSystem').textContent) || 0,
        platesPhysical: parseInt(document.getElementById('eodPlatesPhysical').value) || 0,
        platesVariance: parseInt(document.getElementById('eodPlatesVariance').textContent) || 0
    };

    Object.keys(inventory).forEach(key => {
        const actual = parseFloat(document.getElementById('eodActual_' + key).value) || 0;
        const expected = inventory[key].openingStock + inventory[key].purchases - inventory[key].consumedToday;
        eodData.inventory[key] = {
            opening: inventory[key].openingStock,
            purchases: inventory[key].purchases,
            consumed: inventory[key].consumedToday,
            expected: expected,
            actual: actual,
    ...
    showMessage('✓ EOD Finalized & Saved', 'success');
...
function updateReports() {
    ...
    switch(period) {
        ...
        case 'weekly':
            ...
            break;
        case 'monthly':
            startDate = new Date(today.getFullYear(), today.getMonth(), 1).toLocaleDateString('en-IN');
            endDate = getTodayDateString();
            break;
        case 'quarterly':
            const quarter = Math.floor(today.getMonth() / 3);
            startDate = new Date(today.getFullYear(), quarter * 3, 1).toLocaleDateString('en-IN');
            endDate = getTodayDateString();
            break;
        case 'yearly':
            startDate = new Date(today.getFullYear(), 0, 1).toLocaleDateString('en-IN');
            endDate = getTodayDateString();
            break;
    }
    ...
    const periodOrders = orders.filter(order => {
        return order.date >= startDate && order.date <= endDate && order.status === 'fulfilled';
    });
    ...
    if (tbody) {
        ...
        Object.entries(itemSales).forEach(([name, data]) => {
            ...
            tbody.innerHTML += `<tr><td style="padding:8px;">${name}</td><td style="padding:8px;">${data.qty}</td><td style="padding:8px;">₹${data.revenue.toFixed(2)}</td><td style="padding:8px;">₹${avgPrice.toFixed(2)}</td></tr>`;
        });
    }
}
...
function saveBillFormat() {
    ...
    showMessage('✓ Bill format saved', 'success');
}
...
function printBill() {
    ...
    showMessage('✓ Bill printed', 'success');
}
...
function clearTodayOrders() {
    ...
    if (confirm(`Delete ${count} orders from today?`)) {
        ...
        localStorage.setItem('cobCornOrders', JSON.stringify(orders));
        loadPendingOrdersInline();
        showMessage('✓ Today\'s orders cleared', 'success');
    }
}
...
console.log('✅ COB CORN POS - Updated System V2 Loaded');
console.log('Changes: Payment auto-save, inventory on fulfill, cups tracking, edit items');
